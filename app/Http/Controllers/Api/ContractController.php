<?php

namespace App\Http\Controllers\Api;

use App\Http\Controllers\Controller;
use App\Http\Requests\ContractRequest;
use App\Models\Contract;
use App\Models\ContractInstallment;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;
use Illuminate\Support\Facades\Storage;

use function PHPUnit\Framework\isEmpty;

class ContractController extends Controller
{
    /**
     * Display a listing of the resource.
     */
    public function index()
    {
        $contracts = Contract::with(['project', 'installments'])->paginate(10);
        if (isEmpty($contracts)) {
            return response()->json([
                'message' => 'Table is empty',
                'data'    => $contracts
            ]);
        }
        return response()->json([
            'message' => 'Data fetched',
            'data'    => $contracts,
        ]);
    }

    /**
     * Store a newly created resource in storage.
     */

    public function store(ContractRequest $request)
    {
        DB::beginTransaction();

        try {
            $data = $request->validated();
            $lastId = Contract::latest('id')->value('id') ?? 0;
            $data['contract_number'] = 'CTR-' . str_pad($lastId + 1, 5, '0', STR_PAD_LEFT);
            if ($request->hasFile('contract_file')) {
                $data['contract_file'] = $request
                    ->file('contract_file')
                    ->store('contracts', 'public');
            }
            $contract = Contract::create($data);
            if ($data['payment_type'] === 'installment') {

                $totalInstallments = 0;

                foreach ($request->installments as $index => $item) {

                    ContractInstallment::create([
                        'contract_id'    => $contract->id,
                        'installment_no' => $index + 1,
                        'amount'         => $item['amount'],
                        'due_date'       => $item['due_date'],
                        'status'         => 'pending',
                    ]);

                    $totalInstallments += $item['amount'];
                }
                if ($totalInstallments != $data['amount']) {
                    throw new \Exception('Installments total must equal contract amount.');
                }
            }

            DB::commit();

            return response()->json([
                'message' => 'Contract created successfully',
                'data'    => $contract->load('installments')
            ], 201);

        } catch (\Throwable $e) {

            DB::rollBack();

            return response()->json([
                'message' => 'Contract creation failed',
                'error'   => $e->getMessage()
            ], 500);
        }
    }
    /**
     * Display the specified resource.
     */
    public function show(string $id)
    {
        $contract = Contract::with(['project','installments'])->find($id);
        if (!$contract) {
            return response()->json([
                'message' => 'Not found id: '.$id,
                'data'    => $contract
            ], 404);
        }
        return response()->json([
            'message' => 'Data fetched successfully',
            'data'    => $contract
        ]);
    }

    /**
     * Update the specified resource in storage.
     */

    public function update(ContractRequest $request, $id)
    {
        $contract = Contract::with('installments')->find($id);

        if (!$contract) {
            return response()->json([
                'message' => 'Not found id: ' . $id
            ], 404);
        }

        DB::beginTransaction();

        try {
            $data = $request->validated();
            if ($request->hasFile('contract_file')) {
                if ($contract->contract_file && Storage::disk('public')->exists($contract->contract_file)) {
                    Storage::disk('public')->delete($contract->contract_file);
                }

                $data['contract_file'] = $request
                    ->file('contract_file')
                    ->store('contracts', 'public');
            }
            $contract->update($data);
            if (
                isset($data['payment_type']) &&
                $data['payment_type'] === 'installment'
            ) {
                $contract->installments()->delete();

                $totalInstallments = 0;

                foreach ($request->installments as $index => $item) {
                    ContractInstallment::create([
                        'contract_id'    => $contract->id,
                        'installment_no' => $index + 1,
                        'amount'         => $item['amount'],
                        'due_date'       => $item['due_date'],
                        'status'         => 'pending',
                    ]);

                    $totalInstallments += $item['amount'];
                }
                if ($totalInstallments != $contract->amount) {
                    throw new \Exception('Installments total must equal contract amount.');
                }

            } else {
                $contract->installments()->delete();
            }

            DB::commit();

            return response()->json([
                'message' => 'Contract updated successfully',
                'data'    => $contract->load('installments')
            ], 200);

        } catch (\Throwable $e) {

            DB::rollBack();

            return response()->json([
                'message' => 'Contract update failed',
                'error'   => $e->getMessage()
            ], 500);
        }
    }
    /**
     * Remove the specified resource from storage.
     */
    public function destroy(string $id)
    {
        $contract = Contract::find($id);
        if (!$contract) {
            return response()->json([
                'message' => 'Not found id: '. $id,
                'data'    => $contract,
            ]);
        }
        if ($contract->contract_file) {
            Storage::disk('public')->delete($contract->contract_file);
        }
        $contract->delete();
        return response()->json([
            'message' => 'Centract deleted successfully',
        ]);
    }
}
